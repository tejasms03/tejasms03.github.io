...
export const proyectos = [
  {
    ...
    descripcion:
      "A swarm robotics platform where every robot is independently simulated or driven over Bluetooth in the same workspace, behind a single interface that nothing above the fleet layer branches on. Policies are trained in simulation with deliberate domain randomisation — command latency, motor lag, per-robot gain, and heading bias are all randomised per episode, because a policy trained at zero latency oscillates on real hardware. The coverage reward deliberately says nothing about flocking or spacing, so spreading behaviour has to be discovered rather than hand-written, and a tool layer with JSON schemas lets a language model drive formations directly.",
